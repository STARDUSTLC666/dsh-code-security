...
function riskOfSpec(spec) {
    const s = spec.trim();
    if (s === '')
        return { risk: 'unpinned', note: '未声明版本约束' };
    if (s === '*' || /^latest$/i.test(s) || /^x$/i.test(s))
        return { risk: 'wildcard', note: '版本约束为通配' };
    if (/^(?:https?|git|git+|github:)/i.test(s))
        return { risk: /#([0-9a-f]{7,40})\b/i.test(s) ? 'exact' : 'git', note: /#/.test(s) ? 'git 源已带提交锁定' : 'git 源未锁定到提交' };
    if (/^(?:file|link):/i.test(s))
        return { risk: 'local', note: '本地/链接依赖，发布时会失配' };
    if (/^[0-9]+(?:\.[A-Za-z0-9.*-]+)+$/.test(s) || /^==[0-9]/.test(s))
        return { risk: 'exact', note: '' };
    if (/[<>=~^]/.test(s) || s.includes('||'))
        return { risk: 'range', note: '' };
    return { risk: 'unpinned', note: '无法识别版本约束' };
}
function pushDeps(list, entries, manager, file, kind) {
...
function parseRequirements(text, file) {
    const list = [];
...
function parsePyproject(text, file) {
    const list = [];
...
function parseComposer(text, file) {
    const list = [];
    try {
        const pkg = JSON.parse(text);
        for (const [key, entries] of [['require', 'runtime'], ['require-dev', 'dev']]) {
            pushDeps(list, asRecord(pkg[key]), 'composer', file, entries);
        }
    }
    catch {
        // ignore malformed
...
function parseGemfile(text, file) {
    const list = [];
...
export async function scanDeps(root) {
    const dependencies = [];
    const warnings = [];
    let filesScanned = 0;
    const visit = async (absolute, rel) => {
        ...
        if (stat.isDirectory()) {
            if (DEFAULT_EXCLUDE_DIRS.includes(path.basename(absolute)))
                return;
        ...
        if (base === 'package.json')
            dependencies.push(...parsePackageJson(text, rel));
        else if (base === 'requirements.txt')
            dependencies.push(...parseRequirements(text, rel));
        else if (base === 'pyproject.toml')
            dependencies.push(...parsePyproject(text, rel));
        else if (base === 'go.mod')
            dependencies.push(...parseGoMod(text, rel));
        else if (base === 'Cargo.toml')
            dependencies.push(...parseCargo(text, rel));
        else if (base === 'composer.json')
            dependencies.push(...parseComposer(text, rel));
        else if (base === 'Gemfile')
            dependencies.push(...parseGemfile(text, rel));
    };
    await visit(root, '');
