...
import { languageForFile, RULES } from './rules.js';
import { policyExcludesDir, policyIgnores } from './policy.js';
...
function disabledRules(line, pattern) {
...
function shouldSkipLine(rule, disabled) {
...
function detectHighEntropy(line, lineNumber, file, rel, rule) {
    const findings = [];
    for (const match of line.matchAll(TOKEN_PATTERN)) {
        ...
        if (!/\d/.test(token))
            continue;
        if (shannon(token) < HIGH_ENTROPY_THRESHOLD)
            continue;
...
function makeId(ruleId, file, line, snippet) {
...
export function scanLineSet(entries, rel, policy) {
    ...
    const prevDisable = new Map();
    for (const entry of entries) {
        if (isComment(entry.text))
            prevDisable.set(entry.line + 1, disabledRules(entry.text, DISABLE_NEXT));
    }
    for (const entry of entries) {
        ...
        for (const rule of applied) {
            if (rule.id === 'SEC-304')
                continue;
            if (policyIgnores(policy, rule.id, rel))
                continue;
            if (shouldSkipLine(rule, currentDisable) || shouldSkipLine(rule, prevDisable.get(entry.line)))
                continue;
            if (rule.fileLevel === true)
                continue;
            if (isCommentLine)
                continue;
            if (!rule.patterns.some((pattern) => pattern.test(entry.text)))
                continue;
            findings.push({
...
/** 扫描单个文件内容，按语言与行应用规则。 */
export function scanText(content, file, rel, policy) {
    const lines = content.split(/\r?\n/);
    const entries = lines.map((text, index) => ({ line: index + 1, text }));
...
/** 扫描目录或单个文件。 */
export async function scanPath(options) {
    ...
    const visit = async (absolute, rel) => {
        ...
        if (stat.isDirectory()) {
            if (DEFAULT_EXCLUDE_DIRS.includes(path.basename(absolute)) || policyExcludesDir(policy, rel))
                return;
            let entries;
            try {
