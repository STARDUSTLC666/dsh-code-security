...
export function resolveConfig(config, cwd = process.cwd()) {
    ...
    if (cfg.maxFiles !== undefined) {
        if (typeof cfg.maxFiles !== 'number' || !Number.isFinite(cfg.maxFiles) || cfg.maxFiles <= 0)
            throw new Error('maxFiles 必须是大于 0 的数字。');
        maxFiles = Math.min(100000, Math.max(10, Math.round(cfg.maxFiles)));
    }
    let maxFileBytes = 2 * 1024 * 1024;
    if (cfg.maxFileBytes !== undefined) {
        if (typeof cfg.maxFileBytes !== 'number' || !Number.isFinite(cfg.maxFileBytes) || cfg.maxFileBytes <= 0)
            throw new Error('maxFileBytes 必须是大于 0 的数字（字节）。');
        maxFileBytes = Math.min(20 * 1024 * 1024, Math.max(64 * 1024, Math.round(cfg.maxFileBytes)));
    }
    const failOn = cfg.failOn === 'critical' || cfg.failOn === 'high' || cfg.failOn === 'medium' || cfg.failOn === 'low' ? cfg.failOn : 'medium';
