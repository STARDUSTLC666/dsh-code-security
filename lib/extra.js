...
export function buildExtraTools(cfg, cwd) {
    const stateDir = cfg.stateDir;
    const secureBaseline = {
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                for (const dep of deps) {
                    const row = (dep ?? {});
                    if (row.risk !== 'exact' && row.risk !== 'range')
                        lines.push('- ' + String(row.name ?? '') + ' [' + String(row.risk ?? '') + '] ' + String(row.file ?? ''));
                }
    ...
    secureBaseline.gate = async (exec, next) => {
        const record = (typeof exec === 'object' && exec !== null ? exec : {});
        const approval = record.approval;
        if (approval === undefined)
            return { kind: 'deny', reason: 'secure_baseline 需要确认，但当前环境没有审批通道。' };
        const outcome = await approval.request({ reason: '把当前全部安全问题接受为基线' });
        if (outcome === 'allowed-once')
            return next();
        if (outcome === 'cancelled')
            return { kind: 'deny', reason: '基线接受被取消，未执行。' };
